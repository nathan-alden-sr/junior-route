...
namespace Junior.Route.Http.RequestHeaders
{
	public class DigestAuthorizationHeader
	{
		...
		private DigestAuthorizationHeader(string authScheme, string username, string realm, string nonce, Uri digestUri, string response, string algorithm, string cnonce, string opaque, string messageQop, string nonceCount)
		{
		...
		public static DigestAuthorizationHeader Parse(string headerValue)
		{
			if (headerValue == null)
			{
				return null;
			...
			return new DigestAuthorizationHeader(authScheme, username, realm, nonce, parsedDigestUri, response, algorithm, cnonce, opaque, messageQop, nonceCount);
		}
	}
}
...
{
	public static class JavaScriptBundleWatcherRouteTester
	{
		[TestFixture]
		public class When_changing_bundle
		{
			...
			public void SetUp()
			{
				...
				File.WriteAllText(_path, "function a () { }");
			...
			private JavaScriptBundleWatcherRoute _javaScriptBundleWatcherRoute;

			[Test]
			public void Must_reset_route_resolved_relative_url_hash_in_query_string()
			{
				Assert.That(_javaScriptBundleWatcherRoute.ResolvedRelativeUrl, Is.StringEnding("0fa2d40f50c1a513605a9827dcdaa936"));

				File.WriteAllText(_path, "function b () { }");

				Thread.Sleep(TimeSpan.FromSeconds(2));

				Assert.That(_javaScriptBundleWatcherRoute.ResolvedRelativeUrl, Is.Not.StringEnding("0fa2d40f50c1a513605a9827dcdaa936"));
			}
		}
		...
		public class When_retrieving_css_bundle_response
		{
			...
			public void SetUp()
			{
				...
				_response = _watcherRoute.ProcessResponse(_request);
			}
			...
			private IAssetTransformer _transformer;
			private string _path;
			private JavaScriptBundleWatcherRoute _watcherRoute;
			private IHttpRuntime _httpRuntime;
			private ISystemClock _systemClock;
			...
			public void Must_set_correct_response_properties()
			{
				...
				Assert.That(_response.ContentType, Is.EqualTo("application/javascript"));

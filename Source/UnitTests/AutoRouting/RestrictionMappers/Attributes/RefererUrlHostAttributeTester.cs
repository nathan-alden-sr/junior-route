...
{
	public static class RefererUrlHostAttributeTester
	{
		[TestFixture]
		public class When_mapping_route_restrictions_using_comparer
		{
			...
			public void SetUp()
			{
				_attribute = new RefererUrlHostAttribute("host", RequestValueComparer.CaseSensitiveRegex);
				_route = new Route.Routing.Route("name", Guid.NewGuid(), "relative");
				_container = MockRepository.GenerateMock<IContainer>();
			}

			private RefererUrlHostAttribute _attribute;
		...
		public class When_mapping_route_restrictions_without_using_comparer
		{
			...
			public void SetUp()
			{
				_attribute = new RefererUrlHostAttribute("host1", "host2");
				_route = new Route.Routing.Route("name", Guid.NewGuid(), "relative");
				_container = MockRepository.GenerateMock<IContainer>();
			}

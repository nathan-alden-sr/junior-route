...
{
	public static class UrlAuthorityAttributeTester
	{
		[TestFixture]
		public class When_mapping_route_restrictions_using_comparer
		{
			...
			public void SetUp()
			{
				_attribute = new UrlAuthorityAttribute("authority", RequestValueComparer.CaseSensitiveRegex);
				_route = new Route.Routing.Route("name", Guid.NewGuid(), "relative");
				_container = MockRepository.GenerateMock<IContainer>();
			}

			private UrlAuthorityAttribute _attribute;

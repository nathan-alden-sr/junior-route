...
{
	public static class CaseSensitiveRegexRequestTester
	{
		[TestFixture]
		public class When_testing_if_matching_strings_match
		{
			...
			{
				Assert.That(CaseSensitiveRegexComparer.Instance.Matches(value, requestValue), Is.True);
			}
		}
		...
		public class When_testing_if_non_matching_strings_match
		{
			...
			public void Must_match(string value, string requestValue)
			{
				Assert.That(CaseSensitiveRegexComparer.Instance.Matches(value, requestValue), Is.False);
			}
		}
	}
}